...
def get_order_history(user_email: str) -> list:
    if not user_email:
        raise HistoryError("Missing X-User-Email header.")

    db   = get_db()
    user = get_user_by_email(db, user_email)

    if not user:
        raise HistoryError("User not found.", 404)
...
def get_ai_recommendations(user_email: str) -> dict:
    if not user_email:
        raise HistoryError("Missing X-User-Email header.")
